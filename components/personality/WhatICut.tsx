...
export function WhatICut({ items }: WhatICutProps) {
  return (
    <div className="my-8 bg-warmth-200 border border-border rounded-lg p-6">
      <Caption className="uppercase tracking-wide text-charcoal-600 mb-4 block">
        What I Cut (and why)
      </Caption>
      ...
      <div className="space-y-4">
        {items.map((item, index) => (
          <div key={index} className="flex items-start gap-3">
            <span className="text-charcoal-400 font-bold text-lg flex-shrink-0">✕</span>
            <div>
              <Body size="sm" className="font-medium text-charcoal-700 mb-1">
                {item.what}
              </Body>
